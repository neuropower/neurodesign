...
class design(object):
    ...
    def check_maxrep(self,maxrep):
        ...
        '''
        repcheck = not ''.join(str(e) for e in [0]*maxrep) in ''.join(str(e) for e in self.order)
    ...
    def check_hardprob(self):
        ...
        if not len(self.experiment.P) == len(obsprob):
            ...
            close = np.isclose(np.array(self.experiment.P),np.array(obsprob),atol=0.05)
    ...
    def designmatrix(self):
        ...
        onsetX = [np.floor(x/self.experiment.resolution)*self.experiment.resolution for x in stimonsets]
        stimonsets = onsetX

        # find indices in resolution scale of stimuli
        XindStim = [int(np.where(self.experiment.r_tp==y)[0]) for y in onsetX]
        ...
        for stimulus in xrange(self.experiment.n_stimuli):
            for dur in xrange(stim_duration_tp):
                X_X[np.array(XindStim)+dur,int(stimulus)] = [1 if z==stimulus else 0 for z in self.order]

        # deconvolved matrix in resolution units
        deconvM = np.zeros([self.experiment.n_tp,int(self.experiment.laghrf*self.experiment.n_stimuli)])
        for stim in xrange(self.experiment.n_stimuli):
            for j in xrange(int(self.experiment.laghrf)):
                deconvM[j:,self.experiment.laghrf*stim+j] = X_X[:(self.experiment.n_tp-j),stim]

        # downsample to TR
        idx = [int(x) for x in np.arange(0,self.experiment.n_tp,self.experiment.TR/self.experiment.resolution)]
        ...
        for stim in range(self.experiment.n_stimuli):
             X_Z[:,stim] = deconvM[:,(stim*self.experiment.laghrf):((stim+1)*self.experiment.laghrf)].dot(self.experiment.basishrf)
    ...
    def FcCalc(self,confoundorder=3):
        ...
        for si in xrange(self.experiment.n_stimuli):
            for sj in xrange(self.experiment.n_stimuli):
                for r in np.arange(1,confoundorder+1):
                    Qexp[si,sj,r-1] = self.experiment.P[si]*self.experiment.P[sj]*(self.experiment.n_trials+1)
    ...
    def FfCalc(self):
        ...
        self.Ff = np.sum(abs(np.array(Pobs)-np.array(self.experiment.n_trials*np.array(self.experiment.P))))
    ...
    def countstim(self):
        ...
        if self.duration:
            if not self.restnum == 0:
                blockdurNR = self.restnum*(self.ITImean+self.trial_duration) #duration of block between rest
                blockdurWR = blockdurNR+self.restdur # duration of block including rest
                blocknum = np.floor(self.duration/blockdurWR) #number of blocks
